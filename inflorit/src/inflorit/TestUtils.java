...
public static IMethod getMethod_HelperHack(ClassHierarchy classHierarchy,
        String qualifiedClassName, String methodName,
        String... argsClassName) {
    IMethod res = null;
    for (IClass curClass : classHierarchy) {
        String qualName = getQualifiedName(curClass);
        if (!qualName.equals(qualifiedClassName)) {
            continue;
        }
        for (IMethod curMethod : curClass.getDeclaredMethods()) {
            if (!curMethod.getName().toString().equals(methodName)) {
                continue;
            }
            ...
            for (int i = 0; i < argsClassName.length; i++) {
                ...
                if (!argsClassName[i].equals(curMethod
                        .getParameterType(i + delta).getName()
...
public static void collectReachable(Object start,
        HashSet<PointerKey> reachablePointerKeys,
    ...
    for (Iterator<Object> it = heapGraph.getSuccNodes(start); it.hasNext();) {
        ...
        if (reachablePointerKeys.contains(curSucc)
                || reachableInstanceKeys.contains(curSucc)) {
            continue; // don't want circles
        }
        Assert.assertTrue("FALL", !reachablePointerKeys.contains(curSucc)
                && !reachableInstanceKeys.contains(curSucc));
        ...
        collectReachable(curSucc, reachablePointerKeys,
                reachableInstanceKeys, heapGraph);
    }
}


/**
 * @short gets ONE node that has thins method (in whatever context)
 *          checks there is one andd only one
 *      just for my hello play
 * */
